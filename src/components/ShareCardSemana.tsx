...
import { codigoLegible } from '@/lib/socialmath';
...
export function ShareCardSemana({ datos, width, ref }: CardProps) {
  ...
  return (
    ...
    <View ref={ref} collapsable={false} style={[styles.card, { width, height }]}>
      <LinearGradient colors={[colors.panel, colors.bg, colors.bg]} locations={[0, 0.45, 1]} style={styles.fondo} />
      ...
      <View style={[styles.contenido, { paddingHorizontal: 34 * u, paddingTop: 40 * u, paddingBottom: 36 * u }]}>
        <View style={styles.cabecera}>
          <Text style={[styles.marca, t(19, { letterSpacing: 9 * u })]}>NIVL</Text>
          <Text style={[styles.rotulo, t(9.5, { letterSpacing: 2.6 * u })]}>PARTE DE LA SEMANA</Text>
        </View>

        <View style={styles.identidad}>
          <Avatar size={86 * u} avatarPath={datos.avatarPath} name={datos.name} />
          <Text style={[styles.nombre, t(25, { marginTop: 14 * u, letterSpacing: -0.5 * u })]} numberOfLines={1}>
            {datos.name}
          </Text>
          {datos.equippedTitle ? (
            <Text style={[styles.titulo, t(10.5, { marginTop: 5 * u, letterSpacing: 2.2 * u })]} numberOfLines={1}>
              « {datos.equippedTitle.toUpperCase()} »
            </Text>
          ) : null}
          <Text style={[styles.rotulo, t(10, { marginTop: 7 * u, letterSpacing: 2.4 * u })]} numberOfLines={1}>
            {kind.title} · RANGO {rank} · NIVEL {lvl.level}
          </Text>
        </View>

        <View style={styles.heroe}>
          <Text style={[styles.xp, t(66, { lineHeight: 74 * u })]} numberOfLines={1} adjustsFontSizeToFit>
            +{datos.xpSemana.toLocaleString('es-ES')}
          </Text>
          <Text style={[styles.rotuloClaro, t(11, { letterSpacing: 4 * u, marginTop: 2 * u })]}>XP ESTA SEMANA</Text>
        </View>

        <View>
          <View style={[styles.cifras, { paddingVertical: 16 * u }]}>
            <View style={styles.cifra}>
              <Text style={[styles.cifraValor, t(24)]}>{pct === null ? `${datos.daysActive}/7` : `${pct} %`}</Text>
              <Text style={[styles.rotulo, t(8.5, { letterSpacing: 1.8 * u, marginTop: 5 * u })]}>
            ...
            <View style={styles.cifra}>
              <View style={styles.rachaFila}>
                ...
                <Text style={[styles.cifraValor, t(24), conRacha && styles.oro]}>{datos.streakDays}</Text>
              </View>
              <Text style={[styles.rotulo, t(8.5, { letterSpacing: 1.8 * u, marginTop: 5 * u })]}>
            ...
            {datos.posicion ? (
              <>
                <View style={styles.divisor} />
                <View style={styles.cifra}>
                  <Text style={[styles.cifraValor, t(24)]}>{datos.posicion.split(' ')[0]}</Text>
                  <Text style={[styles.rotulo, t(8.5, { letterSpacing: 1.8 * u, marginTop: 5 * u })]} numberOfLines={1}>
        ...
        <Text style={[styles.lema, t(15, { letterSpacing: 1.2 * u })]}>1 % mejor cada día</Text>

        <View style={[styles.llamada, { paddingVertical: 16 * u, paddingHorizontal: 16 * u }]}>
          <Text style={[styles.rotuloClaro, t(9.5, { letterSpacing: 3 * u })]}>ENTRÉNAME LA DISCIPLINA</Text>
          <Text style={[styles.codigo, t(27, { letterSpacing: 5 * u, marginTop: 8 * u })]}>
            {codigoLegible(datos.friendCode)}
          </Text>
          <Text style={[styles.web, t(13, { letterSpacing: 2.5 * u, marginTop: 8 * u })]}>nivl.app</Text>
        </View>
      </View>
    </View>
  );
  ...
  return (
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <View style={styles.telon}>
        {datos ? <ShareCardSemana ref={lienzo} datos={datos} width={ancho} /> : null}
